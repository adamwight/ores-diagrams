...
\begin{tikzpicture}[
  ...
  \draw[to] (evaluate) to[bend left=50] node[midway, left] {health statistics} (researcher);
  \draw[to] (experiments) -- (wikilabels);
  \draw[to] (researcher) -- node[midway,right] {assumptions \\ choice of data} (experiments);
